...
## Add board events (proposal + tasks review)

- 2026-09-06: Reviewed `add-board-events` proposal.md and tasks.md against the design, spec, and codebase after the user added the security goal ("each user gets events only for his boards"). Verified: `openspec validate add-board-events` passes; all referenced code exists (checkBoardAccess in middleware/auth.ts, audit onResponse hook at app.ts:214, jwt.ts module-level env pattern, reloadBoard/activeBoardId in StoreContext.tsx, api.ts getBoard + /api base, vite proxy, docker-compose.yml, DOCKER-README.md, tests/ layout). Findings: (1) task 1.1 registry `Map<boardId, Set<Socket>>` cannot implement task 3.2 `closeBoardStreams(boardId, userId)` — needs per-user socket mapping; (2) board deletion revokes access but spec/tasks only close streams on member removal — DELETE /boards/:id should `closeBoardStreams(boardId)` instead of emit (an emit would trigger reloadBoard → 404 → user toast, violating the silent best-effort spec); (3) task 4.1 reconnect loop is unbounded (removed member or disabled backend → endless 1 s retry); (4) ticket store has no expiry sweep; (5) actual mutation sites are 18 (boards 3, lists 5, cards 8, members 2), not ~15; (6) optional hardening: bind ticket to boardId at issuance. Not yet applied to artifacts — pending user confirmation.
- Remaining: apply agreed fixes to proposal/tasks/design, then run `/opsx:apply add-board-events`.
Model: DeepSeek v4 Pro [1m]

## Add board events (apply review fixes)

- 2026-09-06: Applied review fixes 1–6 to `add-board-events` (user-approved). (1) Registry now `Map<boardId, Map<userId, Set<Socket>>>` in design decision 12 + task 1.1 so `closeBoardStreams(boardId, userId)` is implementable. (2) Board deletion closes all streams instead of emitting (proposal, design decision 4/12, task 3.2); spec gained "Board deleted" scenario under revoked access. (3) Frontend reconnect loop bounded: capped exponential backoff 1 s → 8 s, permanent stop on 401/404, always silent (design decision 6, task 4.1); spec gained "Revoked access stops reconnecting" scenario under best-effort. (4) Ticket store: purge of expired entries (lazy + periodic sweep, app.ts auditCleanupTimer pattern) in design decision 5 + task 1.2. (5) Mutation count corrected ~15 → 17 emit sites (boards 2, lists 5, cards 8, members 2) across proposal/design/task 3.1. (6) Tickets board-scoped: `POST /api/events/ticket` takes `{ boardId }`, store `Map<ticket, {userId, boardId, expiresAt}>`, stream rejects foreign-board tickets (proposal, design decision 5 + stolen-ticket risk, spec ticket requirement + scenarios, tasks 1.2/2.1/4.1). `openspec validate add-board-events --strict` passes.
- Remaining: run `/opsx:apply add-board-events`.
Model: DeepSeek v4 Pro [1m]

## Add board events (apply)

- 2026-09-06: Implemented `add-board-events` via /opsx:apply. Backend: new `src/events.ts` (per-board/per-user subscriber registry `Map<boardId, Map<userId, Set<Socket>>>`, `emitBoardChange` marking boards dirty, single global flush ticker at `NELLO_EVENTS_INTERVAL_SECONDS` default 3, 15 s heartbeat, `closeBoardStreams`, `closeAllEventStreams`, board-scoped ticket store 120 s TTL consume-on-connect with lazy purge + 60 s sweep, kill switch `NELLO_EVENTS_ENABLED`); new `src/routes/events.ts` (`POST /api/events/ticket` authenticate + checkBoardAccess, `GET /api/boards/:id/events` ticket auth + membership, `config: { rateLimit: false }`, hand-rolled SSE on `reply.raw`); `app.ts` registers routes + timers behind `EVENTS_ENABLED` (onClose cleanup) and skips `/api/events/*` + `/api/boards/:id/events` in the audit onResponse hook. Emit points: 17 `emitBoardChange` calls (boards 2, lists 5, cards 8 incl. both boards on cross-board move, members 2); `closeBoardStreams` on member removal and on board deletion (no emit). Deviation from design decision 5, applied for task 4.1's stop condition: the ticket endpoint also checks board access (EventSource cannot surface the stream's HTTP status, so the ticket request carries the client's 401/404 stop signal); design/tasks/spec updated accordingly (spec gained "Non-member requests a ticket" scenario).
- Frontend: new `src/events.ts` `subscribeBoardEvents` (ticket fetch via `api.requestEventTicket`, EventSource, capped exponential backoff 1 s → 8 s, permanent stop on ticket 401/404, always silent, `VITE_EVENTS_ENABLED` gate); `StoreContext.tsx` effect keyed on activeBoardId with ~100 ms coalescing into `reloadBoard`; `api.ts` gained `requestEventTicket`. Config: `NELLO_EVENTS_ENABLED` + `NELLO_EVENTS_INTERVAL_SECONDS` in `nello/docker-compose.yml`; kill switches + interval documented in `nello/DOCKER-README.md`.

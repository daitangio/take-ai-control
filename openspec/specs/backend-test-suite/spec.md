...
Provide an automated TypeScript test suite for the Nello backend API under `nello/backend/tests/`, runnable via Vitest, that exercises the auth, boards, lists, cards, and members routes against an isolated in-memory database without binding a TCP port. The suite guards the TypeScript/Fastify rewrite against regressions and reproduces the coverage of the original Python pytest suite.
